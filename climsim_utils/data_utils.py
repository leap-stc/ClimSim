...
class data_utils:
    ...
    def get_xrdata(self, file, file_vars=None):
        ...
        if file_vars is not None:
            try:
                ...
                print(
                    f"One or more of the specified variables are not available in the file {file}."
                )
    ...
    def load_ncdata_with_generator(self, data_split):
        ...
        if self.ml_backend == "tensorflow":
            ...
            # Note: removed output_shapes and output_types as with modern tf this is not necessary
            return self.tf.data.Dataset.from_generator(
                gen, output_types=(self.tf.float64, self.tf.float64)
            )
    ...
    def save_as_npy(self, data_split, save_path="", save_latlontime_dict=False):
        ...
        npy_input = np.concatenate(
            [npy_iterator[x][0] for x in range(len(npy_iterator))]
        )
        npy_target = np.concatenate(
            [npy_iterator[x][1] for x in range(len(npy_iterator))]
        )
    ...
    def get_pressure_grid_plotting(self, data_split):
        ...
        ps = np.concatenate(
            [
                self.get_xrdata(file, ["state_ps"])["state_ps"].values[np.newaxis, :]
    ...
    def plot_r2_analysis(self, pressure_grid_plotting, save_path=""):
        ...
        test_heat_daily_long, test_moist_daily_long = self.reshape_daily(
            self.target_scoring
        )
        for i, model_name in enumerate(self.model_names):
            pred_heat_daily_long, pred_moist_daily_long = self.reshape_daily(
                self.preds_scoring[model_name]
            )
            coeff = 1 - np.sum(
                (pred_heat_daily_long - test_heat_daily_long) ** 2, axis=1
            ) / np.sum(
                (
                    test_heat_daily_long
                    - np.mean(test_heat_daily_long, axis=1)[:, None, :]
                )
                ** 2,
                axis=1,
            )
            ...
            contour_plot = ax[0, i].pcolor(
                X, Y, coeff, cmap="Blues", vmin=0, vmax=1
            )  # pcolormesh
            ...
            coeff = 1 - np.sum(
                (pred_moist_daily_long - test_moist_daily_long) ** 2, axis=1
            ) / np.sum(
                (
                    test_moist_daily_long
                    - np.mean(test_moist_daily_long, axis=1)[:, None, :]
                )
                ** 2,
                axis=1,
            )
            ...
            ax[1, i].xaxis.set_ticks(
                [np.sin(-50 / 180 * np.pi), 0, np.sin(50 / 180 * np.pi)]
            )
        ...
        plt.show()
        plt.savefig(
            save_path + "press_lat_diff_models.png",
            bbox_inches="tight",
            pad_inches=0.1,
            dpi=300,
        )
