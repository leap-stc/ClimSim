...
from climsim_datapip_classifier_h5 import climsim_dataset_classifier_h5
from climsim_datapip_classifier import climsim_dataset_classifier
from climsim_unet_classifier import ClimsimUnet_class
import climsim_unet_classifier as climsim_unet_classifier
import hydra
...
class StaticCaptureTrainingWithClip(StaticCaptureTraining):
    def __init__(self, *args, max_grad_norm=1.0, **kwargs):
        super().__init__(*args, **kwargs)
        self.max_grad_norm = max_grad_norm  # Set the maximum gradient norm for clipping
    ...
    def __call__(self, fn: Callable) -> Callable:
        ...
        def decorated(*args: Any, **kwds: Any) -> Any:
            with torch.no_grad() if self.no_grad else nullcontext():
                ...
                if not self.eval:
                    # Apply gradient clipping right before the optimizer step
...
def main(cfg: DictConfig) -> float:
    ...
    val_dataset = climsim_dataset_classifier(input_paths = val_input_path, 
                                  ...
                                  threshold_class1 = cfg.threshold_class1,
                                  threshold_class2 = cfg.threshold_class2,
                                  qn_logtransform = cfg.qn_logtransform)
    ...
    train_dataset = climsim_dataset_classifier_h5(parent_path = cfg.data_path, 
                                    input_sub = input_sub, 
                                    input_div = input_div, 
                                    out_scale = out_scale, 
                                    qinput_prune = cfg.qinput_prune, 
    ...
    model = ClimsimUnet_class(
        num_vars_profile = input_size//60,
        num_vars_scalar = input_size%60,
        num_vars_profile_out = 1,
        num_vars_scalar_out = 0,
        seq_resolution = 64,
    ...
    def cross_entropy_loss(pred, target):
        '''
        pred: (batch_size*level, 3)
        target: (batch_size*level)
        '''
    ...
    if not cfg.clip_grad:
        ...
        raise NotImplementedError('clip_grad is not implemented')
        @StaticCaptureTrainingWithClip(
            model=model,
            optim=optimizer,
            max_grad_norm=cfg.clip_grad_norm  # You can adjust this value as needed
        )
        def training_step(model, data_input, target):
            data_input, target = data_input.to(device), target.to(device)
            output = model(data_input)
        ...
        with LaunchLogger("train", epoch=epoch) as launchlog:
            ...
            for data_input, target in train_loop:
                if cfg.early_stop_step > 0 and current_step > cfg.early_stop_step:
                    break
                # if cfg.qoutput_prune: # this is currently done in the dataset class
                ...
                data_input, target = data_input.to(device), target.to(device)
                if cfg.qn_logtransform:
                    data_input[:,120:180] = torch.where(data_input[:,120:180]<1e-15, 1e-15, data_input[:,120:180])
                    data_input[:,120:180] = torch.log10(data_input[:,120:180])
                    data_input[:,120:180] = torch.clip(data_input[:,120:180], -15, -3)
                    data_input[:,120:180] = (data_input[:,120:180] + 15) / 12
                #optimizer.zero_grad()
    ...
    if dist.rank == 0:
        ...
        climsim_unet_classifier.device = "cpu"
