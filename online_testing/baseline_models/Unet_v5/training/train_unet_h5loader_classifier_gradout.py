...
def main(cfg: DictConfig) -> float:
    ...
    for epoch in range(cfg.epochs):
        ...
        with LaunchLogger("train", epoch=epoch, mini_batch_log_freq=5) as launchlog:
            model.train()
            ...
            for data_input, target in train_loop:
                ...
                if cfg.clip_grad:
                    ...
                    # approx_max_norm = cfg.clip_grad_mean_norm * torch.sqrt(torch.tensor(num_params))
                ...
                launchlog.log_minibatch({"loss_train": loss.detach().cpu().numpy(), "lr": optimizer.param_groups[0]["lr"], "max_grad": max_grad.item(), "mean_grad_l2": mean_l2_norm.item(), "total_norm": total_norm.item()})
